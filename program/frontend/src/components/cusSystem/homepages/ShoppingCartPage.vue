    ...
    <el-card>
        ...
        <el-row :gutter="20">
            <el-col :span="8">
                ...
                >
                    <template #append>
                    <el-button @click="queryFuc" type="danger"><el-icon><search/></el-icon></el-button>
                    </template>
                </el-input>
            </el-col>
            <el-col :span="4">
        ...
        <el-table 
            ...
            <el-table-column label="购买数量" prop="quantity">
                <template  v-slot="scope">
                    <el-input-number v-model="scope.row.quantity" :min="1" :max="scope.row.com_left"  :disabled="scope.row.com_status==-1||scope.row.com_status==0" style="width: 100%;" size="small"/>
            ...
            <el-table-column label="小计" prop="quantity">
                <template  v-slot="scope">
                    <el-text >{{ (scope.row.quantity * scope.row.com_price).toFixed(2) }} </el-text>
                </template> 
            </el-table-column>
        ...
        <el-row justify="space-between">
            ...
            <div>
                <el-button type="danger" plain @click="comfirmIndent" :disabled="multiSelected_row.length==0">购买</el-button>
...
<!-- 确认下单页面 -->
    ...
    <el-card v-for="indent in indent_arr" :key="indent.sto_id " style="margin-top: 10px;">
        ...
        >
            ...
            <el-table-column label="所需金额" prop="ind_money">
                <template  v-slot="scope">
                    <el-text >{{ (scope.row.ind_money).toFixed(2) }} </el-text>
                </template> 
            </el-table-column>
        </el-table>
        <el-input 
    ...
    <h4>您的余额：{{ balance.toFixed(2) }} <br>本单价格：{{ indent_price.toFixed(2) }} <br>您的余额将剩余：<el-text :type="balance-indent_price<0?'danger':'success'">{{ (balance-indent_price).toFixed(2) }} </el-text></h4>
    <template #footer>
      <span class="dialog-footer">
        <el-button @click="indentVisible = false">取消</el-button>
        <el-button type="danger" @click="createIndent">
          确定下单
        </el-button>
      </span>
    </template>
