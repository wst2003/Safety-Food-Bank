        ...
        >
            <el-tab-pane label="商品" name="商品">
                ...
                <el-input v-model="search_str" placeholder="输入你想搜索的商品" clearable>
                    ...
                    <template #append>
                        <el-button @click="onSearchCom" type="primary"><el-icon><search/></el-icon> </el-button>
                    </template>
                </el-input>
            </el-tab-pane>
            <el-tab-pane label="商店" name="商店">
                ...
                <el-input v-model="search_str" placeholder="输入你想搜索的商店" clearable>
                    ...
                    <template #append>
                        <el-button @click="onSearchSto" type="primary"><el-icon><search/></el-icon> </el-button>
                    </template>
                </el-input>
            </el-tab-pane>
        </el-tabs>
            ...
            <Waterfall :list="List" v-bind="options" v-if="listIsCom">
                <template #item="{ item  }" >
                    <div  class="cardset">
                        ...
                        <div>
                            <el-row justify="space-around" @click="handleClickList(item)"> 
                                <el-col :span="10" >
                                    <h3 :style="{ color: item.com_color }" >
                                    {{ item.com_name }}
                                    </h3>
                                </el-col>
                                <el-col :span="3">
                                <h4 style="text-decoration:line-through;" >¥ {{ item.com_oriPrice }}</h4>
                                </el-col>
                                <el-col :span="3" >
                                    <h3 >¥ {{ item.com_price }}</h3 >
                                </el-col>
                            </el-row>
                            <el-row justify="space-around">
                                <el-col :span="14" :offset="2">
                                    <h4>
                                    距离过期还有{{ item.dayDiff }}天 
                                    </h4>
                                </el-col>
                                <el-col :span="4">
                                    <h3 v-if="item.com_status==0" style="color:red;">售罄</h3>
                                </el-col>
                                <el-col :span="4" style="display: flex;justify-content: center;align-items: center;">
                                   
                                    <el-button type="warning" :icon="item.favor_state? StarFilled:Star" circle @click="favorClick(item)"  />
                                </el-col>
                            </el-row>
                        </div>
                    </div>
...
function handleCurrentChange(){
    console.log(JSON.stringify({
                search_str: queryInfo.query,
                com_categories:TagSelected.value,
                begin_pos:10*(queryInfo.pagenum-1),
                end_pos:10*queryInfo.pagenum,
                sort_order: sortSelected.value,
              }))
...
function favorClick(item:Commodity_tab){
    ...
    axios.post(
        ...
        //'http://127.0.0.1:4523/m1/3026709-0-default/api/favorite/setFavorState',
        JSON.stringify({
            com_id:item.com_id,
            cus_id:user_id.value,
            //此处指定收藏的用户，后续更改
